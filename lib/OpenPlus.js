...
OpenPlus = {
    ...
    Polygon: function (center, xAxis, yAxis, sides, radius) {
        ...
        var zAxis = [0, 0, 1];
        var targetAxis = verb.core.Vec.cross(normXaxis, normYaxis);

        var rotAxis = verb.core.Vec.cross(zAxis, targetAxis);

        var angleA = verb.core.Vec.angleBetween(zAxis, targetAxis);
        var rotMatA = this.MatrixAxisAngle(rotAxis, angleA);

        //
        var newXAxis = verb.core.Mat.solve(rotMatA, [1, 0, 0, 1]);
        newXAxis = [newXAxis[0], newXAxis[1], newXAxis[2]];
        newXAxis = verb.core.Vec.normalized(newXAxis);

        var angleB = verb.core.Vec.angleBetween(newXAxis, normXaxis);
        var rotMatB = this.MatrixAxisAngle(targetAxis, angleB);

        var transMat = this.MatrixTranslation(center);

        returnCurve = returnCurve.transform(rotMatA);
        returnCurve = returnCurve.transform(rotMatB);
...
//        console.log(pathsVertices);
    ...
    MatrixAxisAngle: function (axis, angle) {
        var s = Math.sin(angle / 2);
        var x = axis[0] * s;
        var y = axis[1] * s;
        var z = axis[2] * s;
    ...
    MatrixScale: function (x, y, z) {
        return [[x, 0, 0],
            [0, y, 0],
            [0, 0, z]];
    },
    VerbMatrix4ToThreeMatrix4: function (vM) {

        var returnMat = new THREE.Matrix4();
        returnMat.set(
...
//        var tempVecA = verb.core.Vec.mul(10000000000.0, vecA);
        var tempVecA = verb.core.Vec.normalized(vecA);
        var tempVecB = verb.core.Vec.normalized(vecB);

        var angle = verb.core.Vec.angleBetween(tempVecA, tempVecB);
    ...
    TestPlanar: function (points) {
        ...
        for (var i = 0; i < points.length; i++) {
            //string = string + points[i].x +","+ points[i].y +","+ points[i].z +" ";
    ...
    //--------THREE_ADDONS
    flipMaterialIndices: function (geom) {
        var faces = geom.faces;

        faces.forEach(function (face) {
            face.materialIndex = (face.materialIndex + 1) % 2;
        });
    },
    mergeGeoms: function (geoms) {

        var mergedGeom = new THREE.Geometry();

        for (var i = 0; i < geoms.length; i++) {
            var tempGeom = geoms[i];

            mergedGeom.merge(tempGeom, tempGeom.matrix);
        }

        mergedGeom.faceVertexUvs[0] = [];

        return mergedGeom;
    },
    //combine geoms into a single geom
    combineGeom: function (geomsFront, geomsBack) {

        var mergedGeom = new THREE.Geometry();

        for (var i = 0; i < geomsFront.length; i++) {
            var tempGeom = geomsFront[i];

            mergedGeom.merge(tempGeom, tempGeom.matrix, 0);
        }

        for (var i = 0; i < geomsBack.length; i++) {
            var tempGeom = geomsBack[i];

            mergedGeom.merge(tempGeom, tempGeom.matrix, 1);
        }
    ...
    combineMesh: function (geomsFront, geomsBack,
            materialFront, materialBack) {

        var mergedGeom = this.combineGeom(geomsFront, geomsBack);
    ...
    mirrorCombineMesh: function (meshesFront, meshesBack,
        ...
        for (var i = 0; i < meshesFront.length; i++) {
            var tempGeom = meshesFront[i];

            mergedGeom.merge(tempGeom, tempGeom.matrix, 0);

            var tempGeomClone = new THREE.Geometry().copy(tempGeom);

            tempGeomClone.scale(1, -1, 1);

            mergedGeom.merge(tempGeomClone, tempGeomClone.matrix, 1);
        }

        for (var i = 0; i < meshesBack.length; i++) {
            var tempGeom = meshesBack[i];
            ...
            mergedGeom.merge(tempGeomClone, tempGeomClone.matrix, 0);
        }
